...
class Profile extends Component {
    ...
    componentDidMount() {
        let profileName = this.props.match.params.profile;
        const axios = axiosAuthed();
        return axios.get('/api/v1/tracks/' + profileName).then((response) => { console.log(response); this.setState({tracks: response.data.tracks}) })
        .catch(error => {
